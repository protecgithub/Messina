...
{
	/// <summary>
	/// Descrizione di riepilogo per SfogliaRdlPaging2 ruoli Dipartimento.
	/// </summary>
	public class SfogliaRdlPaging2 :  System.Web.UI.Page
	{
		...
		{
			operatore =HttpContext.Current.User.Identity.Name;
			...
			string _mypage="./ManutenzioneCorrettiva/SfogliaRdlPaging1.aspx";			
			...
			{	
				...
				//Richiedenti1.NomeCompleto= operatore.ToString();
		...
		private void Ricerca(bool reset)
		{
			...
			DataSet _MyDs = _Richiesta.GetSfogliaRDLPaging(_SCollection).Copy();	
		...
		private string operatore1()
		{
			...
			if (nr==3)
			{
				if (operatore=="urp")
				{
				operatore="UTENTE URP";
				}
				else{operatore="UTENTE DIPARTIMENTO";}				
			
			}
			if (nr==4)
			{operatore="UTENTE DIPARTIMENTO" + operatore.Substring(3,1);}
			if(nr==5)
			{operatore="UTENTE DIPARTIMENTO" + operatore.Substring(3,2);}
		...
		private S_ControlsCollection GetData()
		{
			...
			//------------------------------------
			s_p_Richiedente.Value = operatore1().ToString();			
		...
		private void cmdExcel_Click(object sender, System.EventArgs e)
		{
			...
			if (cmbsTipoManutenzione.SelectedValue=="3")
			{
				...
				DataSet _MyDs = _Richiesta.GetSfogliaRDLExcel(_SCollection).Copy();	
