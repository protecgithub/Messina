...
var gotoString = "Vai al mese successivo"
var todayString = "Oggi: "
var weekString = "Sett."
var scrollLeftMessage = "Vai al mese precedente."
var scrollRightMessage = "Vai al mese successivo."
var selectMonthMessage = "Click per selezionare il mese."
var selectYearMessage = "Click per selezionare l`anno."
var selectDateMessage = "Selezione: [date] " // do not replace [date], it will be replaced by date.
...
if (dom) {
	...
	document.write("<div onclick='bShow=true' id='calendar'	class='div-style'><table width=" + ((showWeekNumber == 1) ? 250 : 220) + " class='table-style'><tr class='title-background-style' ><td><table width='" + ((showWeekNumber == 1) ? 248 : 218) + "'><tr><td class='title-style'><B><span id='caption'></span></B></td><td align=right><a href='javascript:hideCalendar()'><IMG SRC='" + imgDir + "close.gif' WIDTH='15' HEIGHT='13' BORDER='0' ALT='Chiudi'></a></td></tr></table></td></tr><tr><td class='body-style'><span id='content'></span></td></tr>")
...
function init() {
	...
	if (!ns4) {
		...
		if (showToday == 1) {
			document.getElementById("lblToday").innerText = todayString + dayName[(today.getDay() - startAt == -1) ? 6 : (today.getDay() - startAt)] + ", " + dateNow + " " + monthName[monthNow] + " " + yearNow;
		}

		var sHTML1 = "<span id='spanLeft'  class='title-control-normal-style' onmouseover='this.className=\"title-control-select-style\";window.status=\"" + scrollLeftMessage + "\"' onclick='javascript:decMonth()' onmouseout='clearInterval(intervalID1);swapImage(\"changeLeft\",\"left1.gif\");this.className=\"title-control-normal-style\";window.status=\"\"' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartDecMonth()\",500)'	onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeLeft' SRC='" + imgDir + "left1.gif' width=10 height=11 BORDER=0>&nbsp</span>&nbsp;";
		sHTML1 += "<span id='spanRight' class='title-control-normal-style' onmouseover='this.className=\"title-control-select-style\";window.status=\"" + scrollRightMessage + "\"' onmouseout='clearInterval(intervalID1);swapImage(\"changeRight\",\"right1.gif\");this.className=\"title-control-normal-style\";window.status=\"\"' onclick='incMonth()' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartIncMonth()\",500)'	onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeRight' SRC='" + imgDir + "right1.gif'	width=10 height=11 BORDER=0>&nbsp</span>&nbsp;";
		sHTML1 += "<span id='spanMonth' class='title-control-normal-style' onmouseover='this.className=\"title-control-select-style\";window.status=\"" + selectMonthMessage + "\"' onmouseout='swapImage(\"changeMonth\",\"drop1.gif\");this.className=\"title-control-normal-style\";window.status=\"\"' onclick='popUpMonth()'></span>&nbsp;";
		sHTML1 += "<span id='spanYear'  class='title-control-normal-style' onmouseover='this.className=\"title-control-select-style\";window.status=\"" + selectYearMessage + "\"'	onmouseout='swapImage(\"changeYear\",\"drop1.gif\");this.className=\"title-control-normal-style\";window.status=\"\"'	onclick='popUpYear()'></span>&nbsp;";
...
function constructCalendar() {
	...
	for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++) {
		...
		//selected day
		if ((datePointer == odateSelected) && (monthSelected == omonthSelected) && (yearSelected == oyearSelected)) { sStyle += " selected-day-style"; }
...
function ControllaDataPicker(txtData) {
	var Dat = new Date();
	var anno;
	var mese;
	var giorno;
	...
	if (txtData.length == 10) {
		if (txtData.substring(2, 3) == "/" && txtData.substring(5, 6) == "/") {
			anno = txtData.substring(6, 10);
			if (anno < 1900) {
				return false;
			}
			// ' Il metodo getMonth restituisce i mesi da 0 a 11

			mese = txtData.substring(3, 5) - 1;
			giorno = txtData.substring(0, 2);

			Dat.setFullYear(anno, mese, giorno);

			if (anno != Dat.getFullYear() || mese != Dat.getMonth() || giorno != Dat.getDate()) {
				return false;
			}
		}
		else {
...
function FormattaData(txtData) {
	var giorno;
	var mese;

	Pos = txtData.lastIndexOf("/", i);
	if (Pos < ((txtData.length) - 5)) {
		txtData = "Errore";
		return txtData;
	}
	...
	giorno = txtData.substring(0, 2);
	if (giorno.charAt(1) == "/") {
		giorno = "0" + giorno.substring(0, 1);
		mese = txtData.substring(2, 4);
		if (mese.charAt(1) == "/") {
			mese = "0" + mese.substring(0, 1);
		}
	} else {
		mese = txtData.substring(3, 5);
		if (mese.charAt(1) == "/") {
			mese = "0" + mese.substring(0, 1);
		}
	}
	txtData = giorno + "/" + mese + "/" + txtData.substr((txtData.length - 4), 4);
	return txtData;
}
...
function checkDate(obj) {
	...
	stringa = obj.value;
	testo = scrivi(stringa);
	obj.value = testo;
	if (obj.value != "") {
		DataIns = FormattaData(obj.value);
		if (DataIns == "Errore") {
			alert("Data non Valida.");
			obj.focus();
			return false;
		}
		if (!ControllaDataPicker(DataIns)) {
			alert("Data non Valida.");
			obj.focus();
			return false;
		} else {
